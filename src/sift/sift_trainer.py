...
class SIFTTrainer:
    ...
    def _load_or_create_index(self) -> Tuple[faiss.Index, Dict[str, int], Dict[int, str]]:
    ...
    def select_examples(self, prompt: str, candidates: List[str], n_examples: int = 5) -> List[str]:
...
class MetricsComputer:
    ...
    def compute_metrics(self, outputs: Dict[str, Any], uncertainty: float = None) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'uncertainty': outputs.get('uncertainty', uncertainty if uncertainty is not None else 0.0)
        }
